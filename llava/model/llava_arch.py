...
class LlavaMetaModel(ABC):
    ...
    def encode_images(
        ...
        molecule_fp: Optional[torch.Tensor] = None,
    ):
        ...
        if getattr(self.config, "dynamic_s2", False):
            ...
            molecule_fp_blocks = molecule_fp
            if molecule_fp is not None:
                mol_fp = molecule_fp if isinstance(molecule_fp, torch.Tensor) else torch.as_tensor(molecule_fp)
                if mol_fp.dim() == 1:
                    mol_fp = mol_fp.unsqueeze(0).unsqueeze(0)
                elif mol_fp.dim() == 2:
                    mol_fp = mol_fp.unsqueeze(0)
                block_counts = torch.tensor(
                    [block_size[0] * block_size[1] for block_size in new_block_sizes],
                    device=mol_fp.device,
                )
                if mol_fp.size(0) == len(new_block_sizes):
                    molecule_fp_blocks = mol_fp.repeat_interleave(block_counts, dim=0)
                else:
                    molecule_fp_blocks = mol_fp
            image_features = self.get_mm_projector()(image_features, molecule_fp=molecule_fp_blocks)
            image_features = list(
            ...
            image_features = self.get_vision_tower()(images)
            image_features = self.get_mm_projector()(image_features, molecule_fp=molecule_fp)
...
class LlavaMetaForCausalLM(ABC):
    def _embed(
        ...
        molecule_fp_per_image = None
        if molecule_fp is not None and "image" in media:
            mol_fp = molecule_fp if isinstance(molecule_fp, torch.Tensor) else torch.as_tensor(molecule_fp)
            if mol_fp.dim() == 1:
                mol_fp = mol_fp.unsqueeze(0).unsqueeze(0)
            elif mol_fp.dim() == 2:
                mol_fp = mol_fp.unsqueeze(0)
            mol_fp = mol_fp.to(device=input_ids.device)
            image_token_id = self.tokenizer.media_token_ids["image"]
            num_images_each_instance = (input_ids == image_token_id).sum(dim=-1)
            num_images = int(num_images_each_instance.sum().item())
            if num_images > 0:
                if mol_fp.size(0) == num_images:
                    molecule_fp_per_image = mol_fp
                else:
                    molecule_fp_per_image = mol_fp.repeat_interleave(num_images_each_instance, dim=0)

        # Extract text and media embeddings
        text_embeds = self.llm.model.embed_tokens(input_ids)
        print(">>>using __embed_media_tokens")
        media_embeds = self.__embed_media_tokens(media, media_config, molecule_fp=molecule_fp_per_image)
        ...
        inputs, labels, attention_mask = self.__batchify_sequence(inputs, labels)
        ...
        for name in media:
            if self.training:
                ...
                if media.get(name) is None or len(media[name]) == 0:
                    ...
                    dummy_molecule_fp = None
                    if name == "image":
                        if molecule_fp is not None and molecule_fp.numel() > 0:
                            dummy_molecule_fp = molecule_fp[:1]
                        else:
                            dummy_molecule_fp = torch.full(
                                (1, 1, 4),
                                -1,
                                dtype=torch.long,
                                device=self.device,
                    ...
                    embeds["dummy"].extend(
                        self.encoders[name]([dummy], media_config[name], molecule_fp=dummy_molecule_fp)
                    )
                    continue
            # print(">>>using sam_clip_vision_tower with")
            if name == "image":
                embeds[name] = deque(self.encoders[name](media[name], media_config[name], molecule_fp=molecule_fp))
            else:
    ...
    def generate(
        ...
        **generation_kwargs,
        ...
        images = generation_kwargs.pop("images", None)
        molecule_fp = generation_kwargs.pop("molecule_fp", None)

        if media is None and images is not None:
            media = {"image": [images]}
        ...
        inputs_embeds, _, attention_mask = self._embed(
            input_ids, media, media_config, None, attention_mask, molecule_fp=molecule_fp
        )
...
class LlavaTopDownMetaForCausalLM(LlavaMetaForCausalLM):
    def encode_images_ps3(
        ...
        if "mlp_downsample" in self.mm_projector.config.mm_projector_type:
            ...
            low_res_image_features = self.get_mm_projector()(low_res_image_features, molecule_fp=molecule_fp)
            ...
            for scale_id in range(len(selection_maps)):
                ...
                cur_feature = self.get_mm_projector()(
                    cur_feature,
                    feature_size=selection_maps[scale_id].shape[-2:],
            ...
            low_res_image_features = self.get_mm_projector()(low_res_image_features, molecule_fp=molecule_fp)
            high_res_image_features = self.get_mm_projector()(high_res_image_features, molecule_fp=molecule_fp)
    ...
    def _embed(
        ...
        if molecule_fp is not None and "image" in media:
            ...
            if num_images > 0:
                if mol_fp.size(0) == num_images:
                    ...
                    mol_fp = mol_fp[instance_id_with_image]
                    if mol_fp.size(0) == num_images:
                        molecule_fp_per_image = mol_fp
                    else:
                        molecule_fp_per_image = mol_fp.repeat_interleave(num_images_each_instance, dim=0)
        ...
        media_embeds, top_down_selection_maps, top_down_selection_probs = self.__embed_media_tokens(
            ...
            molecule_fp=molecule_fp_per_image,
        )
