...
class SAMCLIP(PreTrainedModel):
    ...
    def _pixel_values_to_embedding(
        ...
        # Use the model's dtype for consistency (avoid bfloat16/float16 mismatch)
        model_dtype = next(self.sam_model.parameters()).dtype
        
        with torch.no_grad():
            for jdx in range(images_spatial_crop.size(0)):
                # with torch.set_grad_enabled(False):
                patches = images_crop[jdx][0].to(model_dtype)  # Use model's dtype
                image_ori = pixel_values[jdx].to(model_dtype)  # Ensure consistent dtype
    ...
    def _process_image_input(
        ...
        pixel_values = image_input[0]  # Keep original dtype, will be cast in _pixel_values_to_embedding
