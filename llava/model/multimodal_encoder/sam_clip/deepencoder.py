...
# Helper to resolve SAM/CLIP checkpoint paths
def _find_default_sam_clip_ckpt():
    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../../"))
    search_roots = [
        os.path.join(repo_root, "checkpoints", "sam_clip_ckpt", "model_cache"),
        os.path.join(repo_root, "checkpoints", "sam_clip_ckpt"),
    ]
...
def _resolve_ckpt_path(checkpoint):
    if checkpoint:
        ...
        if os.path.exists(checkpoint):
            return checkpoint
        # Try to download from Hugging Face if a repo id is given
        if "/" in checkpoint:
            try:
                from huggingface_hub import snapshot_download

                download_dir = snapshot_download(checkpoint, allow_patterns=["*.safetensors", "*.pt", "*.bin"])
                for pattern in ("*.safetensors", "*.pt", "*.bin"):
                    matches = sorted(glob.glob(os.path.join(download_dir, pattern)))
                    if matches:
                        return matches[0]
            except Exception as e:
                print(f"Warning: failed to download checkpoint '{checkpoint}' from Hugging Face: {e}")
    fallback = _find_default_sam_clip_ckpt()
    if fallback is not None:
        if checkpoint and not os.path.exists(checkpoint):
            print(f"Warning: checkpoint '{checkpoint}' not found. Using fallback '{fallback}'.")
        else:
            print(f"Using fallback checkpoint '{fallback}'.")
        return fallback
    raise FileNotFoundError(
        f"Checkpoint file not found. Looked for '{checkpoint}'. "
        "Please place the SAM/CLIP checkpoint under checkpoints/sam_clip_ckpt/model_cache "
...
def build_clip_l(ckpt=None, device=None, dtype=None):
    ckpt = _resolve_ckpt_path(ckpt)
    model = VitModel(
        cfg=vit_model_cfg,
        freeze_embed=True,
    ...
    # Move to target device/dtype before (re)creating buffers so logs reflect correct placement.
    if device is None:
    ...
    if hasattr(model.embeddings, 'position_ids'):
        ...
        pos_device = next(model.parameters()).device
        # Recreate position_ids buffer on the correct device
        model.embeddings.register_buffer(
            "position_ids", 
            torch.arange(model.embeddings.num_positions).expand((1, -1)).to(pos_device)
        )
        print(f"Reinitialized position_ids buffer on device: {pos_device}")
...
def _build_sam(
    ...
    checkpoint = _resolve_ckpt_path(checkpoint)
