#!/bin/bash
# Stage 2 Specialist: PubChem Caption
# Fine-tune on a single dataset and evaluate on test/valid.

SPECIALIST_NAME="pubchem_cap"
TRAIN_DATASET="vis_chebi20_mix"
TEST_DATASET="vis_chebi20_description_test"
VALID_DATASET="vis_chebi20_description_valid"

PRETRAINED_ckpt=${1:-"./checkpoints/stage1-align/model"}
OUTPUT_DIR=${2:-"./checkpoints/vis-chebi20-${SPECIALIST_NAME}"}
GPUS_PER_NODE=${3:-8}
EXTRA_EVAL_ARGS=("${@:4}")
...
EVAL_OUTPUT_DIR=${EVAL_OUTPUT_DIR:-"./eval_outputs/vis_chebi20_${SPECIALIST_NAME}"}
